...
app.post('/api/site/publish', requireAuth, async function(req, res) { try { var u=await queryOne('UPDATE websites SET published=NOT published, published_at=CASE WHEN NOT published THEN NOW() ELSE published_at END, status=CASE WHEN NOT published THEN $2 ELSE $3 END WHERE professional_id=$1 RETURNING *', [req.session.professionalId, 'published', 'draft']); res.json({ success:true, published:u.published }); } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); } });
