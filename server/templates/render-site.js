...
function renderSite(site, slug, websiteRow) {
  ...
  const data = {
    ...
    google_maps_url_real: !!(w.google_maps_url && w.google_maps_url !== '#'),
    google_rating_real:    !!(w.google_rating && parseFloat(w.google_rating) > 0),
    google_rating_display: w.google_rating ? w.google_rating : '',
    google_stars_display:  w.google_rating ? '★★★★★' : '★★★',
    google_pill_label:     w.google_rating ? ((w.google_review_count || '') + ' avis Google') : 'Nos clients nous recommandent',
    show_reviews:          true,
